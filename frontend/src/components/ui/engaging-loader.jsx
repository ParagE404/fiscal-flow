...
export const FinancialLoader = ({ className, size = "default", ...props }) => {
      ...
      <div className="relative">
        {icons.map((item, index) => {
          ...
          return (
            ...
            >
              <Icon
                className={cn(sizes[size], color, isActive && "animate-bounce")}
              />
            </div>
          );
        })}
  ...
  return (
    <div className={cn("flex items-end gap-1", className)} {...props}>
      {Array.from({ length: bars }).map((_, i) => (
        <div
          key={i}
          className={cn("w-1 rounded-full animate-pulse", colors[color])}
          style={{
...
export const MorphingLoader = ({ className, size = "default", ...props }) => {
  const sizes = {
    sm: "w-6 h-6",
    default: "w-8 h-8",
    lg: "w-12 h-12",
...
export const ShimmerSkeleton = ({
  ...
  return (
    <div
      className={cn("shimmer rounded-md", variants[variant], className)}
      {...props}
    />
  );
};
...
export const AnimatedLoadingCard = ({
  ...
  return (
    <div
      className={cn("modern-card p-8 text-center space-y-6", className)}
      {...props}
    >
