...
class PortfolioStore {
  ...
  get totalPortfolioValue() {
    ...
    const mfValue =
      this.mutualFundsSummary.totalCurrentValue ||
      (Array.isArray(this.mutualFunds)
        ? this.mutualFunds.reduce(
            (sum, fund) =>
              sum + (fund.totalCurrentValue || fund.currentValue || 0),
            0
          )
        : 0);
    const fdValue = Array.isArray(this.fixedDeposits)
      ? this.fixedDeposits.reduce((sum, fd) => sum + (fd.currentValue || 0), 0)
      : 0;
    const epfValue = Array.isArray(this.epfAccounts)
      ? this.epfAccounts.reduce((sum, epf) => sum + (epf.totalBalance || 0), 0)
      : 0;
    const stockValue = Array.isArray(this.stocks)
      ? this.stocks.reduce((sum, stock) => sum + (stock.currentValue || 0), 0)
      : 0;
  ...
  get totalInvested() {
    // Use summary data if available, otherwise calculate from individual funds
    const mfInvested =
      this.mutualFundsSummary.totalInvestment ||
      (Array.isArray(this.mutualFunds)
        ? this.mutualFunds.reduce(
            (sum, fund) =>
              sum + (fund.totalInvestment || fund.investedAmount || 0),
            0
          )
        : 0);
    const fdInvested = Array.isArray(this.fixedDeposits)
      ? this.fixedDeposits.reduce(
          (sum, fd) => sum + (fd.investedAmount || 0),
          0
        )
      : 0;
    const epfInvested = Array.isArray(this.epfAccounts)
      ? this.epfAccounts.reduce(
          (sum, epf) => sum + (epf.employeeContribution || 0),
          0
        )
      : 0;
    const stockInvested = Array.isArray(this.stocks)
      ? this.stocks.reduce((sum, stock) => sum + (stock.investedAmount || 0), 0)
      : 0;
  ...
  get totalReturnsPercentage() {
    ...
    return (
      ((this.totalPortfolioValue - this.totalInvested) / this.totalInvested) *
      100
    );
  }
  ...
  get topPerformers() {
    ...
    if (Array.isArray(this.mutualFunds)) {
      this.mutualFunds.forEach((fund) => {
        const totalCurrentValue =
          fund.totalCurrentValue || fund.currentValue || 0;
        const totalInvestment =
          fund.totalInvestment || fund.investedAmount || 0;
    ...
    if (Array.isArray(this.fixedDeposits)) {
      this.fixedDeposits.forEach((fd) => {
        ...
        const returnsPercentage =
          fd.investedAmount > 0 ? (returns / fd.investedAmount) * 100 : 0;
    ...
    if (Array.isArray(this.epfAccounts)) {
      this.epfAccounts.forEach((epf) => {
        ...
        const totalContributions =
          (epf.employeeContribution || 0) + (epf.employerContribution || 0);
        const returns = (epf.totalBalance || 0) - totalContributions;
        const returnsPercentage =
          totalContributions > 0 ? (returns / totalContributions) * 100 : 0;
    ...
    return performers
      .filter(
        (performer) =>
          performer.returnsPercentage !== 0 || performer.returns !== 0
      )
      .sort((a, b) => {
  ...
  async addMutualFund(fundData) {
    ...
    try {
      ...
      runInAction(() => {
        this.mutualFunds = this.mutualFunds.filter(
          (fund) => fund.id !== tempId
        );
      });
      this.setError("mutualFunds", this.getErrorMessage(error));
  ...
  async refreshAllData(force = false) {
    ...
    try {
      ...
      await Promise.race([Promise.all(refreshPromises), timeoutPromise]);
  ...
  validateDataConsistency() {
    ...
    this.mutualFunds.forEach((fund, index) => {
      if (!fund.id || !fund.name) {
        issues.push(
          `Invalid mutual fund at index ${index}: missing required fields`
        );
      }
    ...
    if (optimisticItems.length > 0) {
      issues.push(
        `Found ${optimisticItems.length} orphaned optimistic updates`
      );
    }
