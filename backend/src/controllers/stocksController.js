...
const { formatIndianCurrency, formatPercentage, formatPnL } = require('../utils/formatting')
...
const getAllStocks = async (req, res, next) => {
  try {
    ...
    // Calculate summary
    const totalInvestment = stocks.reduce((sum, stock) => sum + stock.investedAmount, 0)
    ...
    // Count by market cap
    const largeCapCount = stocks.filter(stock => stock.marketCap === 'Large Cap').length
    const midCapCount = stocks.filter(stock => stock.marketCap === 'Mid Cap').length
    const smallCapCount = stocks.filter(stock => stock.marketCap === 'Small Cap').length
    ...
    const stocksWithCalculations = stocks.map(stock => {
      const pnlData = calculateStockPnL(stock.quantity, stock.buyPrice, stock.currentPrice)
...
const createStock = async (req, res, next) => {
  try {
    ...
    const pnlData = calculateStockPnL(stockData.quantity, stockData.buyPrice, stockData.currentPrice)
...
const updateStock = async (req, res, next) => {
  try {
    ...
    if (updateData.quantity || updateData.buyPrice || updateData.currentPrice) {
      const quantity = updateData.quantity || existingStock.quantity
      const buyPrice = updateData.buyPrice || existingStock.buyPrice
      const currentPrice = updateData.currentPrice || existingStock.currentPrice
